# Sentinel — Known Limitations (v1.2.0)

_Updated for v1.2.0. Every accepted V1 limitation, its consequence, and its
...
Sentinel v1.2.0 remains deliberately small. The limitations below are **accepted decisions with
...
| 19  | Tenant and anonymous identities are hashed, not encrypted                    | A SHA-256 hash is not a secret; keys/logs carry identity hashes, never raw tenant, cookie, or IP values       | `sentinel/limiter.py:37`, `sentinel/observability.py:59`   |
...
Redis. Under load, on slow hosts, or across a wide network, `REDIS_TIMEOUT` is a _normal,
classified_ outcome that routes through the §06 decision table — design dashboards and alerting
...
named _per process_. Additionally, emergency buckets are keyed by `endpoint_id` only — tenant
...
The sliding-window counter returns an _estimate_ — `current + previous × (remaining/window)`
...
There is no replica or failover path in v1.2.0. When Redis is down, fail-closed endpoints are
