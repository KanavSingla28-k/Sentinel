...
import pytest
from sentinel.redis import NOEVICTION_POLICY, SentinelRedis
...
async def test_bench_smoke_runs_and_reports_sane_statistics(
    ...
    previous = {
        "maxmemory-policy": (await redis_client.client.config_get("maxmemory-policy")).get(
            "maxmemory-policy"
        ),
        "maxmemory": (await redis_client.client.config_get("maxmemory")).get("maxmemory"),
    }
    try:
        ...
        await redis_client.client.config_set("maxmemory-policy", previous["maxmemory-policy"])
        await redis_client.client.config_set("maxmemory", previous["maxmemory"])
